...
def create_all_count_maps(infile, nside, raname, decname, wname, zname, zbins, catformat):
    """Create count maps binned in redshift from input galaxy catalogs.
    ...
    """
    data = Table.read(infile, format=catformat)  # Don't try/except, astropy already does the job.
    ...
    if zbins is not None:
        assert (isinstance(zbins, list)
                and all(isinstance(zbin, tuple) and len(zbin) == 2 for zbin in zbins)
                and all(isinstance(elem, (int, float)) for zbin in zbins for elem in zbin)), (
    ...
    if (zname is None) and (zbins is None):
        zname = "fake_column"
...
def main(infolder, nside, maskpath, outdir, outfileroot, raname, decname, wname, zname, zbins,
    ...
    # ------ Read input catalogs and run map creation in parallel
    ...
    lambda_create_count_maps = partial(create_all_count_maps, nside=nside, raname=raname,
                                       decname=decname, wname=wname, zname=zname, zbins=zbins,
                                       catformat=catformat)
    parallel_count_maps = processors.map(lambda_create_count_maps, inpaths)

    # ------ Postprocess maps - Merge counts, transform to overdensities and save output
